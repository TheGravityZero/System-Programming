...
//)
double day__of__week(){
        return zeller(dd, (mm < 3. ? (mm + 10.): mm - 2.), (remainder(mm < 3. ? (yyyy - 1.): yyyy, 100.)), (quotient(mm < 3. ? (yyyy - 1.): yyyy, 100.)));
}
...
//)
double birthday(double dw){
        display("Your were born on "); newline();
        display(dw == 1. ? "Monday ": dw == 2. ? "Tuesday ": dw == 3. ? "Wednesday ": dw == 4. ? "Thursday ": dw == 5. ? "Friday ": 
        dw == 6. ? "Saturday ": "Sunday "); display(dd); display("."); display(mm); display(".");
        return  yyyy;
}
